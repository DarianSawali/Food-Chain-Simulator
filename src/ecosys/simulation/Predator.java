...
public class Predator extends Animal{
	...
	private Color pRed;
	private Ellipse2D.Double midBod;
	...
	@Override
	protected boolean eatable(SimulationObject prey) {
		return (prey instanceof Prey && prey.size < size); 
	}
	
//	public boolean sees(Prey b) {
...
//		g2.draw(fov);
	...
	protected void setShapeAttributes() {
		//midBod = new Ellipse2D.Double(-dim.width/2, -dim.height/2, dim.width/2, 2*dim.height/4);
		body = new Ellipse2D.Double(-5*dim.width/4, -dim.height/2, dim.width, dim.height);
	...
	protected void setOutline() {
		...
		outline.add(new Area(stripe1));
		outline.add(new Area(stripe2));
	...
	protected Shape getFOV() {
	    AffineTransform az = new AffineTransform();
	    az.translate(pos.x, pos.y);
	    az.rotate(speed.heading());
	    az.scale(size, size);
	    return az.createTransformedShape(fov);
	}
}
